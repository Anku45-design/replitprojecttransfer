import { useLiveDashboard, useLiveDistricts, useLiveAlerts } from "@/hooks/use-flood-data";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { formatNumber, cn } from "@/lib/utils";
import { Activity, AlertTriangle, Users, MapPin, Waves, CloudRain, ShieldAlert } from "lucide-react";
...
export default function Dashboard() {
  ...
  if (loadingSummary) {
    return <div className="flex items-center justify-center h-full text-muted-foreground animate-pulse font-mono">INITIALIZING DATALINK...</div>;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">Real-time flood risk analytics and active monitoring.</p>
        </div>
      </div>
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={trendData}>
                  ...
                  />
                  <Line yAxisId="left" type="monotone" dataKey="level" name="Level (m)" stroke="#3b82f6" strokeWidth={3} dot={false} activeDot={{ r: 6 }} />
                  <Line yAxisId="right" type="monotone" dataKey="rain" name="Rain (mm)" stroke="#0ea5e9" strokeWidth={2} strokeDasharray="5 5" dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <Card className="flex flex-col">
          ...
          <CardContent className="p-0 overflow-y-auto max-h-[350px]">
            {alerts?.slice(0, 5).map(alert => (
              <div key={alert.id} className="p-4 border-b border-border/50 hover:bg-muted/30 transition-colors">
                <div className="flex justify-between items-start mb-2">
                  <span className="font-semibold text-sm">{alert.districtName}, {alert.state}</span>
              ...
              <tbody>
                {topDistricts.map((d, i) => (
                  <tr key={d.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                    <td className="px-6 py-4 font-medium">{d.name}, <span className="text-muted-foreground font-normal">{d.state}</span></td>
                    <td className="px-6 py-4">
                      <Badge variant={d.riskLevel}>{d.riskLevel}</Badge>
                    </td>
                    <td className="px-6 py-4 text-right font-mono text-foreground">{d.riskScore}/100</td>
                    ...
                    <td className="px-6 py-4 text-right font-mono">
                      <div className={d.riverLevel > d.dangerLevel ? "text-critical" : "text-foreground"}>
                        {d.riverLevel}m <span className="text-muted-foreground text-xs ml-1">(Danger: {d.dangerLevel}m)</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-right font-mono">{formatNumber(d.populationAffected)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function StatCard({ title, value, icon: Icon, valueClass = "text-foreground", trend }: { title: string, value: string | number, icon: any, valueClass?: string, trend?: string }) {
  return (
    <Card>
      <CardContent className="p-6">
        <div className="flex justify-between items-start">
          <div className="space-y-2">
            ...
            <p className={cn("text-3xl font-display font-bold", valueClass)}>{value}</p>
          </div>
